...
A new **bullish fair value gap**: a candle whose low is above the high of the candle two before, a gap left by a fast push up. The H1 structure is up.

Questions are only asked between 10:00 and 20:00 server time (London open to late New York).

## Why pros take it
A gap like that shows buyers in a hurry. With the trend behind it, pros expect continuation (or a retest of the gap that holds).
...
- The gap was made by a news spike that is already reversing.
- It comes at the end of a long run up (exhaustion).
- Thin, quiet markets, where one order can leave a gap.
...
## What the agent sees
`fvg_net10`, `h1_structure`, `body`, EMA distances, `atr_rel`, the chart. All 49 indicator inputs and the 178 chart inputs go in; these are the ones that matter most here.
