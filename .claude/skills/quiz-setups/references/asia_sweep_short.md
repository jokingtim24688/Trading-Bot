...
Price pushed **more than 0.3 ATR over the Asian range high** within the last 15 candles, then closed back more than 0.1 ATR below it.

Questions are only asked between 10:00 and 20:00 server time (London open to late New York).

## Why pros take it
A raid of the stops above the Asian high that fails: pros sell the reversal.
...
- A trending day: the Asian high is taken and held.
- A news-driven raid.
- A very narrow Asian range.
...
## What the agent sees
`asia_hi_dist`, `h1_structure`, time of day (`tod_sin`/`tod_cos`), `atr_rel`. All 49 indicator inputs and the 178 chart inputs go in; these are the ones that matter most here.
