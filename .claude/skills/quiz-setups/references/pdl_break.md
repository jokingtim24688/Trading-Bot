...
The first close **more than 0.3 ATR below yesterday's low**, with the H1 structure down.

Questions are only asked between 10:00 and 20:00 server time (London open to late New York).

## Why pros take it
Acceptance below yesterday's low: longs there are wrong and breakout sellers join.
...
- A quick return inside the range (a sweep).
- A late or quiet break.
- A $50 round number just below.
...
## What the agent sees
`pdl_dist`, `h1_structure`, `round_big_dist`, time of day (`tod_sin`/`tod_cos`), `atr_rel`. All 49 indicator inputs and the 178 chart inputs go in; these are the ones that matter most here.
