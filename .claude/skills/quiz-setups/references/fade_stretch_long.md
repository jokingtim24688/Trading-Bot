...
Price is **more than 4 ATR below the session average** and just printed a strong up candle (body more than half an ATR).

Questions are only asked between 10:00 and 20:00 server time (London open to late New York).

## Why pros take it
Moves that far from the day's average tend to snap back toward it. The strong candle is the first sign.
...
- A trend day or a news sell-off: stretched can get more stretched.
- One up candle inside a waterfall (no pause before it).
- This is counter-trend, so traps are more common than for trend setups.
...
## What the agent sees
`sess_avg_dist`, `body`, `h1_structure`, `atr_rel`, the chart. All 49 indicator inputs and the 178 chart inputs go in; these are the ones that matter most here.
