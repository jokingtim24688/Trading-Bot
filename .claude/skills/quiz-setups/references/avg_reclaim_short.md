...
The H1 structure is down and price **falls back below the session average**.

Questions are only asked between 10:00 and 20:00 server time (London open to late New York).

## Why pros take it
In a downtrend, pops above the day's average are where sellers step back in. Losing it again shows they have.
...
## What the agent sees
`sess_avg_dist`, `h1_structure`, EMA distances, `atr_rel`. All 49 indicator inputs and the 178 chart inputs go in; these are the ones that matter most here.
