...
- **run.py**: loop; `broker.sync()` every second; cheap poll for a new closed bar; paper default; `--live` demo only unless `--allow-real`; modes paper/demo/real; netting guard; early exit when the model flips (`close_one(..., "early")`); records prob, risk_money and open_bar per trade; STOP file kill switch. Scores the last 1440 candles on start (no warm-up). `--settings` makes it re-read the threshold, `bot_mode` and the co-pilot timer every candle. `plan_for()`/`enter()` place the order; `propose()`/`check_proposal()` run co-pilot. The status adds `headline`, `confluence`, `confidence_pct` and `proposal` (without SL).
- **livecard.py**: `confluence()`, `confidence_pct()`, `headline()` and `reason()`: the Bot tab's plain words.
...
- **learn.py**: analyze ledger → learned_rules.json + `.claude/skills/m1-bot-lessons/`; `block_reason()` used by run.py; live, its confidence bars are capped at the model's own recent readings (`recent_probs`: median for min_confidence, 80th pct for cautions), so a bar learned on another scale can't block every entry.
